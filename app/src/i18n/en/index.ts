export default {
    // Common
    common: {
        save: 'Save',
        cancel: 'Cancel',
        delete: 'Delete',
        edit: 'Edit',
        add: 'Add',
        search: 'Search',
        loading: 'Loading...',
        error: 'Error',
        success: 'Success',
        confirm: 'Confirm',
        back: 'Back',
        close: 'Close',
        yes: 'Yes',
        no: 'No',
        noData: 'No data',
        amount: 'Amount',
        date: 'Date',
        description: 'Description',
        name: 'Name',
        type: 'Type',
        actions: 'Actions',
        all: 'All',
        apply: 'Apply',
    },

    // Validation
    validation: {
        required: 'This field is required',
        minLength: 'Minimum {min} characters',
        maxLength: 'Maximum {max} characters',
        invalidEmail: 'Invalid email',
        invalidNumber: 'Invalid number',
        positiveNumber: 'Number must be positive',
    },

    // Navigation
    nav: {
        report: 'Report',
        transactions: 'Transactions',
        investments: 'Investments',
        projects: 'Projects',
        more: 'More',
    },

    // More menu
    more: {
        budgets: 'Budgets',
        categories: 'Categories',
        masterCategories: 'Master Categories',
        wallets: 'Wallets',
        games: 'Games',
        recurring: 'Recurring',
        netWorth: 'Net Worth',
        goals: 'Goals',
        settings: 'Settings',
        analytics: 'Analytics',
        management: 'Management',
        categorization: 'Categorization',
        planning: 'Planning',
        analysis: 'Analysis',
        application: 'Application',
    },

    // Transactions
    transactions: {
        title: 'Transactions',
        income: 'Income',
        expense: 'Expense',
        transfer: 'Transfer',
        project: 'Project',
        injection: 'Injection',
        dividend: 'Dividend',
        addIncome: 'Add Income',
        addExpense: 'Add Expense',
        addTransfer: 'Add Transfer',
        addProject: 'Add Project Transaction',
        addInjection: 'Add Injection',
        addDividend: 'Add Dividend',
        addTransaction: 'Add Transaction',
        editTransaction: 'Edit Transaction',
        noTransactions: 'No transactions yet',
        balance: 'Balance',
        transactionsCount: 'transactions',
        fromWallet: 'From Wallet',
        toWallet: 'To Wallet',
        sourceWallet: 'Source Wallet',
        destinationWallet: 'Destination Wallet',
        transferFee: 'Transfer Fee',
        transferFeeHint: 'Fee charged for this transfer (optional)',
        fee: 'Fee',
        today: 'Today',
        yesterday: 'Yesterday',
        deleteTitle: 'Delete Transaction',
        deleteMessage: 'This transaction will be deleted and wallet balances will be reverted.',
        deleteConfirm: 'Are you sure you want to delete this transaction?',
        incomeBreakdown: 'Income Breakdown',
        expenseBreakdown: 'Expense Breakdown',
    },

    // Wallets
    wallets: {
        title: 'Wallets',
        balance: 'Balance',
        initialBalance: 'Initial Balance',
        initialBalanceHint: 'The amount currently available in this wallet',
        addWallet: 'Add Wallet',
        editWallet: 'Edit Wallet',
        noWallets: 'No wallets yet',
        deleteConfirm: 'Are you sure you want to delete this wallet?',
        deleteWarning: 'Associated transactions will not be deleted.',
        icon: 'Icon',
        types: {
            cash: 'Cash',
            bank: 'Bank Account',
            mobile_money: 'Mobile Money',
            investment: 'Investment',
            other: 'Other',
        },
    },

    // Categories
    categories: {
        title: 'Categories',
        category: 'Category',
        addCategory: 'Add Category',
        editCategory: 'Edit Category',
        noCategories: 'No categories yet',
        deleteConfirm: 'Are you sure you want to delete this category?',
        deleteWarning: 'Associated transactions will not be deleted.',
        masterCategory: 'Master Category',
        icon: 'Icon',
    },

    // Master Categories
    masterCategories: {
        title: 'Master Categories',
        addMasterCategory: 'Add Master Category',
        editMasterCategory: 'Edit Master Category',
        noMasterCategories: 'No master categories yet',
        deleteConfirm: 'Are you sure you want to delete this master category?',
        deleteWarning: 'Associated categories will not be deleted.',
        icon: 'Icon',
        color: 'Color',
        selectIcon: 'Select an icon',
        selectColor: 'Select a color',
        types: {
            expense: 'Expense',
            income: 'Income',
        },
    },

    // Budgets
    budgets: {
        title: 'Budgets',
        addBudget: 'Add Budget',
        editBudget: 'Edit Budget',
        noBudgets: 'No budgets yet',
        month: 'Month',
        spent: 'Spent',
        remaining: 'Remaining',
        exceeded: 'Exceeded',
        copyPrevious: 'Copy from previous month',
        totalBudgeted: 'Total Budgeted',
        byCategory: 'By Category',
        byMasterCategory: 'By Master Category',
        deleteConfirm: 'Are you sure you want to delete this budget?',
        exceededWarning: '{count} budget(s) exceeded',
    },

    // Investments
    investments: {
        title: 'Investments',
        addInvestment: 'Add Investment',
        editInvestment: 'Edit Investment',
        noInvestments: 'No investments yet',
        totalInvested: 'Total Invested',
        totalValue: 'Total Value',
        currentValue: 'Current Value',
        gainLoss: 'Gain/Loss',
        quantity: 'Quantity',
        rate: 'Rate',
        currentRate: 'Current Rate',
        newRate: 'New Rate',
        updateRate: 'Update Rate',
        health: 'Health',
        evolution: 'Evolution',
        label: 'Investment Name',
        sourceWallet: 'Source Wallet',
        wallet: 'Wallet',
        buy: 'Buy',
        sell: 'Sell',
        pricePerUnit: 'Price per Unit',
        total: 'Total',
        available: 'Available',
        availableBalance: 'Available Balance',
        insufficientQuantity: 'Insufficient quantity',
        insufficientBalance: 'Insufficient balance',
        transactions: 'Transactions',
        noTransactions: 'No transactions',
        noHistory: 'Not enough data for chart',
        items: 'assets',
        units: 'units',
        deleteConfirm: 'Are you sure you want to delete this investment?',
        notFound: 'Investment not found',
        types: {
            stock: 'Stock',
            crypto: 'Crypto',
            other: 'Other',
        },
    },

    // Projects
    projects: {
        title: 'Projects',
        addProject: 'Add Project',
        editProject: 'Edit Project',
        noProjects: 'No projects yet',
        totalInvested: 'Total Invested',
        dividendsReceived: 'Dividends Received',
        roi: 'ROI',
        returnOnInvestment: 'Return on Investment',
        netGain: 'Net Gain',
        projectCount: 'projects',
        invested: 'Invested',
        history: 'Transaction History',
        noTransactions: 'No transactions yet',
        transactionCount: 'Transactions',
        deleteConfirm: 'Are you sure you want to delete this project?',
    },

    // Games (betting platforms)
    games: {
        title: 'Games',
        name: 'Game name',
        icon: 'Icon',
        addGame: 'Add Game',
        editGame: 'Edit Game',
        noGames: 'No games yet',
        walletCreatedHint: 'A dedicated wallet will be created automatically for this game.',
        excludedHint:
            'Game balances are excluded from your reports: the money may be locked on the platform.',
        balance: 'Available Balance',
        netResult: 'Net Result',
        totalStaked: 'Total Staked',
        totalWon: 'Total Won',
        pendingStake: 'Pending Stakes',
        pendingBets: 'pending bets',
        deposit: 'Deposit',
        withdraw: 'Withdraw',
        withdrawAmount: 'Withdrawn amount',
        netReceived: 'Amount received: {amount}',
        withdrawFeeTooHigh: 'Fee must be lower than the withdrawn amount',
        deleteConfirm:
            'Are you sure you want to delete this game? Its wallet and bets will be deleted.',
    },

    // Bets
    bets: {
        addBet: 'Place a Bet',
        editBet: 'Edit Bet',
        resolveBet: 'Settle Bet',
        history: 'Bet History',
        noBets: 'No bets yet',
        stake: 'Stake',
        payout: 'Payout',
        pending: 'Pending',
        won: 'Won',
        lost: 'Lost',
        deleteConfirm:
            'Are you sure you want to delete this bet? The game balance will be adjusted.',
    },

    // Report / Dashboard
    report: {
        title: 'Report',
        selectMonth: 'Select Month',
        totalBalance: 'Total Balance',
        income: 'Income',
        expenses: 'Expenses',
        net: 'Net',
        expenseDistribution: 'Expense Distribution',
        categoryDistribution: 'By Category',
        totalExpenses: 'Total Expenses',
        details: 'Details',
        noExpenses: 'No expenses this month',
        investments: 'Investments',
        projects: 'Projects',
        revenueDistribution: 'Revenue Distribution',
        investedInStocks: 'Invested in Stocks/Crypto',
        investedInProjects: 'Invested in Projects',
        spent: 'Spent',
        savings: 'Savings',
        budgetAlerts: 'Budget Alerts',
        byMasterCategory: 'By Master Category',
    },

    // Analytics
    analytics: {
        title: 'Analytics',
        subtitle: 'Filter and break down your spending',
        filters: 'Filters',
        included: 'Included flows',
        groupBy: 'Group by',
        masterCategories: 'Master categories',
        categories: 'Categories',
        wallets: 'Wallets',
        amountMin: 'Min amount',
        amountMax: 'Max amount',
        amountHint: '0 = no limit',
        searchDescription: 'Search in descriptions',
        resetFilters: 'Reset',
        total: 'Total',
        totalSpent: 'Total spent',
        dailyAverage: 'Daily average',
        totalOutflow: 'Total outflow',
        ofWhichSpent: 'spent',
        ofWhichAllocated: 'allocated',
        allocated: 'Allocated',
        allocatedHint: 'projects, games',
        largestOutflow: 'Largest outflow',
        flows: 'Entries',
        savingsRate: 'Savings rate',
        vsPreviousPeriod: 'vs previous period',
        topExpenses: 'Largest expenses',
        flowCount: '{count} transaction(s)',
        noData: 'No data for this period',
        other: 'Other',
        unknown: 'Unknown',
        quarterShort: 'Q',
        granularity: 'Granularity',
        evolution: 'Evolution',
        cumulative: 'Cumulative',
        trend: 'Trend',
        movingAverage: '{window}-period average',
        average: 'Average',
        highest: 'Highest',
        lowest: 'Lowest',
        perPeriod: 'Per period',
        incomeVsExpenses: 'Income vs expenses',
        netPerPeriod: 'Net per period',
        detailed: 'Detail by master category',
        comparisonScopeHint:
            'Income and expenses are always included here, whatever the flow filters say.',
        tabs: {
            breakdown: 'Breakdown',
            evolution: 'Evolution',
            comparison: 'Comparison',
        },
        granularities: {
            week: 'Week',
            month: 'Month',
            quarter: 'Quarter',
        },
        presets: {
            currentMonth: 'This month',
            last3Months: '3 months',
            last6Months: '6 months',
            last12Months: '12 months',
            yearToDate: 'This year',
            custom: 'Custom',
        },
        groups: {
            expense: 'Expenses',
            income: 'Income',
            game: 'Games',
            project: 'Projects',
            fee: 'Transfer fees',
            unknown: 'Uncategorized',
        },
        dimensions: {
            masterCategory: 'By master category',
            category: 'By category',
            wallet: 'By wallet',
        },
    },

    // AI report export
    aiReport: {
        title: 'AI analysis',
        subtitle: 'Send a summary of my finances to an assistant',
        disclaimer:
            'This data leaves your phone. The assistant you pick may retain it and use it for training. Review the text before sending.',
        window: 'Period to send',
        months: '{count} months',
        includeAmounts: 'Include amounts',
        includeAmountsHint: 'Off: only shares and rates are sent, no amounts.',
        anonymizeLabels: 'Anonymize labels',
        anonymizeLabelsHint:
            'Replaces category names with "Master category 1, 2, 3…". Transaction labels are never included.',
        largeExpenseThreshold: 'Large expense threshold',
        largeExpenseThresholdHint: 'Expenses above it are listed one by one',
        preview: 'Text to send (editable)',
        characters: '{count} characters',
        copy: 'Copy',
        send: 'Share with…',
        shareWith: 'Send to an assistant',
        sent: 'Report shared',
        copied: 'Report copied',
        copiedAndOpened: 'Report copied, paste it in the chat',
        failed: 'Sharing failed',
    },

    // Net Worth
    netWorth: {
        title: 'Net Worth',
        subtitle: 'Track how your wealth evolves over time',
        total: 'Total Net Worth',
        liquid: 'Liquidity',
        investments: 'Investments',
        games: 'Games',
        breakdown: 'Breakdown',
        evolution: 'Evolution',
        history: 'History',
        captureNow: 'Capture now',
        captured: 'Snapshot captured',
        noData: 'Not enough data yet',
        noHistory: 'No snapshots yet',
        vsPrevious: 'vs previous month',
        deleteConfirm: 'Are you sure you want to delete this snapshot?',
    },

    // Recurring transactions
    recurring: {
        title: 'Recurring',
        subtitle: 'Automate fixed income and expenses',
        addRecurring: 'Add Recurring',
        editRecurring: 'Edit Recurring',
        noRecurring: 'No recurring transactions yet',
        frequency: 'Frequency',
        weekly: 'Weekly',
        monthly: 'Monthly',
        yearly: 'Yearly',
        interval: 'Interval',
        intervalHint: 'Repeat every N periods',
        dayOfMonth: 'Day of month',
        startDate: 'Start date',
        endDate: 'End date',
        endDateOptional: 'End date (optional)',
        autoPost: 'Auto-post transactions',
        autoPostHint: 'Automatically create the transaction when due',
        reminderOnly: 'Reminder only',
        active: 'Active',
        inactive: 'Inactive',
        nextOccurrence: 'Next',
        upcoming: 'Upcoming',
        projectedMonthEnd: 'Projected month-end balance',
        projectedHint: 'Current balance plus upcoming recurring income and expenses',
        deleteConfirm: 'Are you sure you want to delete this recurring transaction?',
        everyWeek: 'Every {count} week(s)',
        everyMonth: 'Every {count} month(s)',
        everyYear: 'Every {count} year(s)',
    },

    // Savings goals
    goals: {
        title: 'Goals',
        subtitle: 'Save toward your targets',
        addGoal: 'Add Goal',
        editGoal: 'Edit Goal',
        noGoals: 'No goals yet',
        name: 'Goal name',
        target: 'Target',
        current: 'Saved',
        remaining: 'Remaining',
        wallet: 'Linked wallet',
        walletHint: 'Progress tracks this wallet balance',
        deadline: 'Target date',
        deadlineOptional: 'Target date (optional)',
        reached: 'Reached',
        onTrack: 'On track',
        behind: 'Behind',
        requiredMonthly: 'To save',
        perMonth: '/mo',
        monthsLeft: '{count} month(s) left',
        noDeadline: 'No deadline',
        walletMissing: 'Linked wallet deleted',
        deleteConfirm: 'Are you sure you want to delete this goal?',
    },

    // Settings
    settings: {
        title: 'Settings',
        language: 'Language',
        currency: 'Currency',
        defaultCurrency: 'Default Currency',
        exchangeRates: 'Exchange Rates',
        theme: 'Theme',
        darkMode: 'Dark Mode',
        exportData: 'Export Data',
        importData: 'Import Data',
        about: 'About',
        currencyLocked: 'Default currency cannot be changed',
        addExchangeRate: 'Add Exchange Rate',
        editExchangeRate: 'Edit Exchange Rate',
        fromCurrency: 'From Currency',
        toCurrency: 'To Currency',
        rate: 'Rate',
        noExchangeRates: 'No exchange rates defined',
        reminder: 'Daily Reminder',
        reminderDescription: 'Reminder at 9:30 PM if no transaction recorded',
        reminderEnabled: 'Reminder enabled',
        reminderDisabled: 'Reminder disabled',
        alarmPermissionRequired: 'Exact alarm permission required',
        openAlarmSettings: 'Open settings',
        securityData: 'Security & Data',
    },

    // App lock
    appLock: {
        title: 'App Lock',
        description: 'Require a PIN to open the app',
        setPin: 'Set PIN',
        changePin: 'Change PIN',
        disable: 'Disable lock',
        disableHint: 'Enter your PIN to disable the lock',
        enterPin: 'Enter your PIN',
        enterCurrentPin: 'Enter current PIN',
        newPin: 'Enter a new PIN',
        confirmPin: 'Confirm your PIN',
        wrongPin: 'Wrong PIN',
        pinMismatch: 'PINs do not match',
    },

    // Backup
    backup: {
        export: 'Export',
        import: 'Import',
        exportTitle: 'Export backup',
        importTitle: 'Import backup',
        exportCaption: 'Encrypted backup file',
        importCaption: 'Restore from a backup file',
        exportHint: 'Your data is exported as a password-encrypted file.',
        importHint: 'Select a Zenkash backup file and enter its password.',
        importWarning: 'Importing replaces all current data.',
        password: 'Password',
        confirmPassword: 'Confirm password',
        chooseAndImport: 'Choose file & import',
        passwordTooShort: 'Password must be at least {min} characters',
        passwordMismatch: 'Passwords do not match',
        wrongPassword: 'Wrong password',
        invalidFile: 'Invalid backup file',
    },

    // Onboarding
    onboarding: {
        welcomeTitle: 'Welcome to Zenkash',
        welcomeSubtitle: 'Manage your personal finances, investments, and projects with ease.',
        getStarted: 'Get Started',
        currencyTitle: 'Choose Your Currency',
        currencyDescription:
            'This currency will be used by default for all your calculations and reports.',
        currencyWarning: 'This choice is permanent and cannot be changed.',
    },

    // Messages
    messages: {
        saved: 'Saved successfully',
        deleted: 'Deleted successfully',
        error: 'An error occurred',
        confirmDelete: 'Are you sure you want to delete this item?',
    },

    // Currencies
    currencies: {
        XOF: 'CFA Franc (BCEAO)',
        XAF: 'CFA Franc (BEAC)',
        EUR: 'Euro',
        USD: 'US Dollar',
        GBP: 'British Pound',
        MAD: 'Moroccan Dirham',
        NGN: 'Nigerian Naira',
        GHS: 'Ghanaian Cedi',
        KES: 'Kenyan Shilling',
        ZAR: 'South African Rand',
    },
};
